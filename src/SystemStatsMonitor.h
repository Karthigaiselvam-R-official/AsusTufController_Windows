...
public slots:
  void start() {
    ...
    // On Windows, MTP devices often show up as Drives or Portable Devices via
    // WPD We will just emit empty or simulate
    emit devicesFound({});
  }
signals:
...
class SystemStatsMonitor : public QObject {
  ...
  Q_PROPERTY(QStringList gpuModels READ gpuModels NOTIFY gpuModelsChanged)

  // Complex Data
  Q_PROPERTY(
      QVariantList diskPartitions READ diskPartitions NOTIFY statsChanged)
...
public:
  ...
  Q_INVOKABLE void
  openFileManager(const QString &mount,
                  const QString &device); // Overload for drive click
...
signals:
  ...
  void statsChanged();
  void gpuModelsChanged(); // New Signal
